...
3. **Distribución de motores — dos vías, según licencia y tamaño.** Cuando toques un motor Wine (parche, DLL nueva, builtin), decide DÓNDE va y actualízalo:
   - **Independencia total.** CrossOver puede usarse para comparar resultados durante el I+D, pero **nunca** como dependencia: producción no busca `CrossOver.app`, no importa su configuración ni ejecuta sus binarios. La misma regla se aplica a Homebrew Wine y a instalaciones globales de GPTK; los motores efectivos deben estar gestionados por Vessel.
   - **Motor base completo → `SwonDev/Vessel-Engines` (release nueva).** SOLO para motores **propios y redistribuibles**: `wine-unified` (WineHQ 11.10 limpio, LGPL) y `wine-full` (build propia de las **fuentes FOSS de CrossOver 26.2.0**, wine-11.0 + CW HACKs, LGPL — release `engine-full-v2`; ver `docs/ENGINE-SOURCE.md` §1b). Si cambias el motor base de forma que un drop-in NO puede parchear (recompilar Wine entero, nuevo builtin, subir de versión de WineHQ), publica una release nueva de Vessel-Engines (`engine-unified-vN`/`engine-full-vN`), sube el `.tar.zst`, y **actualiza la URL en el código** (`DependencyManager`/`WineEngineLocator`). **NUNCA** subas el Wine copiado de un CrossOver.app instalado (la copia histórica `wine-full-crossover-bak` contiene `winewrapper.exe`, `apple_gptk` y herramientas `cx*`, con licencia CodeWeavers) ni `gptk-mythic`: el `.gitignore` los bloquea. La regla práctica para saber cuál es cuál: el `wine-full` redistribuible **no tiene** `lib/wine/x86_64-windows/winewrapper.exe` (ver `WineEngineLocator.containsExternalFullEngineRuntime`).
   - **Fix puntual → drop-in en `Resources/engine-*fix/` (dentro del `.app`).** Para cualquier arreglo que se pueda aplicar en caliente sobre un motor ya instalado: `applyCryptoFix` (gnutls/freetype), `applyDDrawFix` (ddraw de War Wind), `applySteamRenderFix` (winemac/win32u), `repairFullEngineShim` (lanzador de wine-full). Van con marcador de versión (idempotentes) y evitan re-subir 540 MB por cada cambio. Es la vía por defecto para fixes de `wine-full`. Verificar SIEMPRE restaurando el original y dejando que Vessel se auto-repare.
...
1. **`steam.cfg` con `BootStrapperInhibitAll=enable`** (`WineManager.ensureSteamConfig`). Sin esto, cuando Steam se relanza solo (sin `-noverifyfiles`) verifica sus ficheros, detecta el wrapper como "corrupto", intenta autoactualizar el cliente, la descarga falla bajo Gcenx (`http error 0`) y queda ladrillado → **`Failed to load steamui.dll`**. EXCEPCIÓN: bajo el motor unificado el updater SÍ funciona (`WINEMSYNC=0`), y `launchSteam` quita el steam.cfg a propósito (modo `needsSelfUpdate`) para actualizar un cliente antiguo una única vez.
   ...
   ⚠️ **El inhibidor SOLO vale sobre un cliente COMPLETO.** Aplicado sobre un bootstrap a medias produce el mismo `Failed to load steamui.dll`, pero **sin salida**: Steam no puede terminar de instalarse ni verificarse, y como el estado se realimenta el error se repite en cada arranque. Por eso `isSteamBootstrapped` NO se conforma con que exista `steamui.dll` (aparece mucho antes de que el cliente esté entero): `WineManager.isSteamClientFullyBootstrapped` exige `steamui.dll` con contenido **+** `steamclient(64).dll` **+** un `bin/cef/cef.win*/steamwebhelper.exe`. Y `ensureSteamConfig` se niega a escribir el inhibidor sobre un cliente incompleto: lo RETIRA, de modo que un bottle ya dañado se repara solo. Red de seguridad adicional: `repairBrickedSteamClientIfNeeded` busca la huella `Failed to load steamui.dll` en `Steam/logs/*.txt` y, si la encuentra, devuelve el bootstrapper y la verificación durante ese arranque (y limpia esos logs para no repetirlo). Corolario: **nunca interrumpas un bootstrap en marcha** — `ensureSteamReadyForLogin` espera con la misma holgura que `updateSteamClient` (hasta 15 min) y, si no termina, deja a Steam trabajando en vez de matarlo.
